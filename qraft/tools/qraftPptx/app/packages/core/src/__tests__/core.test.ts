...
beforeEach(async () => {
  tempRoot = await mkdtemp(path.join(os.tmpdir(), "qraftPptx-"));
  core = new MicroKeynoteCore({ centralRoot: path.join(tempRoot, "qraft", "tools", "qraftPptx"), defaultPort: 3456 });
...
describe("MicroKeynoteCore", () => {
  ...
  it("rejects symlink-style client roots that point into app source through registry validation", async () => {
    const appLink = path.join(tempRoot, "projects", "tapwise", "tools", "qraftPptx");
