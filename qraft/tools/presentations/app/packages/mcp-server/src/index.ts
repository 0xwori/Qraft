...
register("list_clients", "List registered Presentations clients.", {}, true, async () => ({ clients: await core.listClients() }));
...
register("launch_ui", "Launch the local Presentations browser UI.", {
...
function registerResources() {
  server.registerResource("clients", "presentations://clients", {
    title: "Presentations Clients",
    mimeType: "application/json",
  }, async (uri) => resourceResult(uri.href, { clients: await core.listClients() }));

  server.registerResource("layouts", "presentations://layouts", {
    title: "Presentations Layout Registry",
    mimeType: "application/json",
  }, async (uri) => resourceResult(uri.href, { layouts: await core.listLayouts() }));

  server.registerResource("themes", "presentations://themes", {
    title: "Presentations Theme Registry",
    mimeType: "application/json",
  }, async (uri) => resourceResult(uri.href, { themes: await core.listThemes() }));

  server.registerResource("client-context", new ResourceTemplate("presentations://client/{clientId}/context", { list: undefined }), {
    title: "Presentations Client Context",
    mimeType: "application/json",
  }, async (uri, variables) => resourceResult(uri.href, await core.readContext(String(variables.clientId))));

  server.registerResource("deck", new ResourceTemplate("presentations://deck/{clientId}/{deckId}", { list: undefined }), {
  ...
  server.registerResource("slide", new ResourceTemplate("presentations://deck/{clientId}/{deckId}/slide/{slideId}", { list: undefined }), {
    title: "Presentations Slide",
    mimeType: "application/json",
  }, async (uri, variables) => {
  ...
  server.registerResource("validation", new ResourceTemplate("presentations://validation/{clientId}/{deckId}", { list: undefined }), {
    title: "Presentations Deck Validation",
    mimeType: "application/json",
  }, async (uri, variables) => {
    const deck = await core.readDeck(String(variables.clientId), String(variables.deckId));
